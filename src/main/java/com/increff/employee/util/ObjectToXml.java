...
public class ObjectToXml {
    ...
    public static void generateXmlString(List<OrderItemData> placeOrderDataList, OrderPojo orderPojo) throws Exception {
        ...
        ArrayList<InventoryPojo.OrderInvoicePojo> orderInvoicePojoList = new ArrayList<>();

        Double totalAmount = 0.0;
        Integer number = 1;
        for(OrderItemData placeOrderData : placeOrderDataList) {
            InventoryPojo.OrderInvoicePojo orderInvoicePojo = new InventoryPojo.OrderInvoicePojo();
        ...
        JAXBContext contextObj = JAXBContext.newInstance(OrderForm.OrderInvoice.class);

        Marshaller marshallerObj = contextObj.createMarshaller();
        marshallerObj.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);

        OrderForm.OrderInvoice orderInvoice = new OrderForm.OrderInvoice(1, orderInvoicePojoList, orderPojo.getTime().toString(), orderPojo.getOrderId(), totalAmount);
